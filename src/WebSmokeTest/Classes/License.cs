...
namespace SmokeTest.Classes
{
    internal class License : ILicense
    {
        ...
        public License()
        {
            ...
            MaximumConfigurations = DefaultMaximumConfiguration;
            MaximumOpenEndpoints = DefaultMaximumOpenEndpoints;
            MaximumPageScans = DefaultMaximumPageScans;
            MaximumRunningTests = DefaultRunningTests;
            MaximumTestsToRun = DefaultMaximumTestsToRun;
            MaximumTestSchedules = DefaultMaximumTestSchedules;
        }
        ...
        internal static ILicense CreateLicense(in ILicenseFactory licenseFactory, in string licenseData)
        {
            ...
            try
            {
                using (MemoryStream ms = new MemoryStream(Convert.FromBase64String(DecryptString(licenseData, Convert.FromBase64String(key)))))
                {
                    using (BinaryReader binaryReader = new BinaryReader(ms))
                    {
                        ...
                        if (CompareByteArrays(header, Header))
                        {
                            ...
                            Result.MaximumRunningTests = binaryReader.ReadInt32();
                            Result.MaximumConfigurations = binaryReader.ReadInt32();
                            Result.MaximumPageScans = binaryReader.ReadInt32();
                            Result.MaximumOpenEndpoints = binaryReader.ReadInt32();
                            Result.MaximumTestsToRun = binaryReader.ReadInt32();
                            Result.MaximumTestSchedules = binaryReader.ReadInt32();
                ...
                if (!licenseFactory.LicenseIsValid(Result))
                {
                    Result.MaximumConfigurations = DefaultMaximumConfiguration;
                    Result.MaximumOpenEndpoints = DefaultMaximumOpenEndpoints;
                    Result.MaximumPageScans = DefaultMaximumPageScans;
                    Result.MaximumRunningTests = DefaultRunningTests;
                    Result.MaximumTestsToRun = DefaultMaximumTestsToRun;
                    Result.MaximumTestSchedules = DefaultMaximumTestSchedules;
                }
            }
        ...
        public int MaximumRunningTests { get; private set; }

        public int MaximumConfigurations { get; private set; }

        public int MaximumPageScans { get; private set; }

        public int MaximumOpenEndpoints { get; private set; }

        public int MaximumTestsToRun { get; private set; }

        public DateTime Expires { get; private set; }

        public string RegisteredUser { get; private set; }

        public int MaximumTestSchedules { get; private set; }
